...
for(i=1:7992)  
    ...
    Z(2*i-1:2*i,1:3)=[qpp1(i) qpp2(i)*cos(q1(i)-q2(i))-(qp2(i)^2)*sin(q2(i)-q1(i)) 0;
        0 qpp1(i)*cos(q2(i)-q1(i))+(qp1(i)^2)*sin(q2(i)-q1(i)) qpp2(i)];
...
for(i=1:7992) 
    %% couple d'inertie
    ciner(2*i-1,1)= p1*qpp1(i)+p2*qpp2(i)*cos(q2(i)-q1(i)); %% AXE 1 
    ciner(2*i,1)=p2*cos(q2(i)-q1(i))*qpp1(i)+p3*qpp2(i); %%AXE 2
    %% couple centrifuge
    ccentri(2*i-1,1)=-p2*sin(q1(i)-q2(i))*(qp2(i))^2; %% AXE 1
    ccentri(2*i,1)= p3*sin(q2(i)-q1(i))*(qp1(i))^2;
    %% couple de gravité
